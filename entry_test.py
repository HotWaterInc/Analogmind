...
def eval_data_changes(storage: StorageSuperset2, seen_network: any) -> any:
    ...
    for connection in sampled_connections:
        ...
        # same_position_difference += torch.cdist(start_embeddings, start_embeddings).mean().item()
        # different_position_difference += torch.cdist(start_embeddings, end_embeddings).mean().item()
        ...
        # different_position_difference += torch.norm(start_embeddings[0] - end_embeddings[0], p=2, dim=0).item()
...
if __name__ == "__main__":
    ...
    abstraction_network = load_manually_saved_ai("abstraction_network_from_exploration250.pth")
    eval_data_changes(storage, abstraction_network)

    connections = _get_connection_distances_seen_network(storage, abstraction_network)

    # Extract the data we need
    filtered_connections = [
        connection for connection in connections
        # if connection["distance_real"] <= 0.5
    ]
