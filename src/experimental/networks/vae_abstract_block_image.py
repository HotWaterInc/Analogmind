...
from src.save_load_handlers.ai_models_handle import save_ai_manually, load_manually_saved_ai
...
def adjacent_distance_handling(autoencoder: BaseAutoencoderModel, adjacent_sample_size: int,
    ...
    """
    sampled_pairs = storage_struct.sample_adjacent_datapoints_connections(adjacent_sample_size)
    ...
    for pair in sampled_pairs:
        ...
        data_point1 = storage_struct.get_datapoint_data_tensor_by_name_permuted(pair["start"])
        data_point2 = storage_struct.get_datapoint_data_tensor_by_name_permuted(pair["end"])
...
def non_adjacent_distance_handling(autoencoder: BaseAutoencoderModel, non_adjacent_sample_size: int,
    ...
    """
    sampled_pairs = storage_struct.sample_datapoints_adjacencies_cheated(non_adjacent_sample_size)

    batch_datapoint1 = []
    batch_datapoint2 = []

    for pair in sampled_pairs:
        datapoint1 = storage_struct.get_datapoint_data_tensor_by_name_permuted(pair["start"])
        datapoint2 = storage_struct.get_datapoint_data_tensor_by_name_permuted(pair["end"])
...
def reconstruction_handling_with_freezing(autoencoder: BaseAutoencoderModel,
    ...
    global storage_struct, recons_data
    sampled_count = 25
...
def train_vae_abstract_block(autoencoder: BaseAutoencoderModel, epochs: int,
    ...
    storage_struct.build_permuted_data_random_rotations_rotation0()
    train_data = array_to_tensor(np.array(storage_struct.get_pure_permuted_raw_env_data())).to(device)
...
storage_struct: StorageSuperset2 = StorageSuperset2()
