...
from src.modules.pretty_display import set_pretty_display, pretty_display_start, pretty_display_reset, pretty_display
...
def embedding_policy(data, rotation: int, index: int):
    global autoencoder
    # start_embedding = data

    global models
    model = models[rotation]
    embedding = model.encoder_inference(data.unsqueeze(0)).squeeze(0)
    return embedding
...
def direction_loss(direction_network, sample_rate=64):
    ...
    for datapoint in datapoints:
        ...
        for j in range(len(connections_to_point)):
            ...
            start_index = storage.get_datapoint_index_by_name(start)
            end_index = storage.get_datapoint_index_by_name(end)
            ...
            start_rotation = metadata[start_index]
            end_rotation = metadata[end_index]

            start_embedding = embedding_policy(start_data, start_rotation, start_index)
            end_embedding = embedding_policy(end_data, end_rotation, end_index)
...
def train_direction_ai(direction_network, num_epochs):
    optimizer = optim.Adam(direction_network.parameters(), lr=0.001)
    ...
    set_pretty_display(epoch_print_rate, "Epochs progress")
    pretty_display_start(0)

    for epoch in range(num_epochs):
        if (epoch % 5 == 0):
            # randi = np.random.randint(0, 23)
            # storage.build_permuted_data_random_rotations_rotation_N(randi)
    ...
    for iter in range(ITERATIONS):
        ...
        for datapoint in datapoints:
            ...
            for j in range(len(connections_to_point)):
                ...
                start_rotation = storage.get_pure_permuted_raw_env_metadata_array_rotation()[start_index]
                end_rotation = storage.get_pure_permuted_raw_env_metadata_array_rotation()[end_index]

                start_embedding = embedding_policy(start_data, start_rotation, start_index).unsqueeze(0)
                end_embedding = embedding_policy(end_data, end_rotation, end_index).unsqueeze(0)
...
def run_new_ai():
    ...
    for i in range(ENSEMBLE_SIZE):
        model = load_manually_saved_ai(f"autoencod_image_ensemble{i}.pth")
        model.eval()
        model = model.to(device)
        models.append(model)

    train_direction_ai(direction_network, num_epochs=1000)
    save_ai_manually("direction_ensemble", direction_network)
...
def run_direction_network_ensemble():
    global storage
    global permutor
    global autoencoder
