...
def train_autoencoder_position_predictor(autoencoder: BaseAutoencoderModel, epochs: int,
    ...
    position_data = array_to_tensor(np.array(storage_raw.get_pure_xy_permuted_raw_env_data()))
    ...
    for epoch in range(num_epochs):
        if (epoch % SHUFFLE_RATE == 0):
            ...
            train_data = array_to_tensor(np.array(storage_raw.get_pure_permuted_raw_env_data())).to(device=device)
            position_data = array_to_tensor(np.array(storage_raw.get_pure_xy_permuted_raw_env_data())).to(device=device)
